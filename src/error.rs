...
pub enum NtHiveError {
    ...
    #[error("The {datatype} at offset {offset:#010x} should have a size of 0, 2, 4 or 8 bytes, but it has {actual} bytes")]
    InvalidWordSize {
        datatype: String,
        offset: usize,
        actual: usize,
    ...
    #[error("While extracting {origin}, the size field at offset {offset:#010x} specifies {expected} bytes, but only {actual} bytes are left in the slice")]
    InvalidSizeField {
        origin: String,
        offset: usize,
        expected: usize,
