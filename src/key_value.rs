    ...
    B: SplitByteSlice,
{
    pub(crate) fn new(hive: &'h Hive<B>, cell_range: Range<usize>) -> Result<Self> {
        ...
        //remove signature as we parse data in a best effort
        Ok(key_value)
    }
    ...
    pub fn dword_data(&self) -> Result<u64> {
        ...
        if let KeyValueData::Small(data) = self.data()? {
            ...
            match data.len() {
                0 => Ok(0),
                1 => Ok(data[0] as u64),
                2 => match self.data_type()? {
                    KeyValueDataType::RegDWord => {
                        Ok(u16::from_le_bytes(data.try_into().unwrap()) as u64)
                    }
                    KeyValueDataType::RegDWordBigEndian => {
                        Ok(u16::from_be_bytes(data.try_into().unwrap()) as u64)
                    }
                    _ => unreachable!(),
                },
                4 => match self.data_type()? {
                    KeyValueDataType::RegDWord => {
                        Ok(u32::from_le_bytes(data.try_into().unwrap()) as u64)
                    }
                    KeyValueDataType::RegDWordBigEndian => {
                        Ok(u32::from_be_bytes(data.try_into().unwrap()) as u64)
                    }
                    _ => unreachable!(),
                },
                8 => match self.data_type()? {
                    KeyValueDataType::RegDWord => {
                        Ok(u64::from_le_bytes(data.try_into().unwrap()) as u64)
                    }
                    KeyValueDataType::RegDWordBigEndian => {
                        Ok(u64::from_be_bytes(data.try_into().unwrap()) as u64)
                    }
                    _ => unreachable!(),
                },
                len => Err(NtHiveError::InvalidWordSize {
                    datatype: "dword".to_string(),
            ...
            Err(NtHiveError::InvalidWordSize {
                datatype: "dword".to_string(),
                offset: self
                    .hive
    ...
    pub fn qword_data(&self) -> Result<u64> {
        ...
        if let KeyValueData::Small(data) = self.data()? {
            match data.len() {
                0 => Ok(0),
                1 => Ok(data[0] as u64),
                2 => Ok(u16::from_le_bytes(data.try_into().unwrap()) as u64),
                4 => Ok(u32::from_le_bytes(data.try_into().unwrap()) as u64),
                8 => Ok(u64::from_le_bytes(data.try_into().unwrap()) as u64),
                len => Err(NtHiveError::InvalidWordSize {
                    datatype: "qword".to_string(),
                    offset: self.hive.offset_of_field(&data),
                    actual: len,
                }),
            }
    ...
    /// Returns the data type code of this Key Value.
    pub fn data_type_code(&self) -> u32 {
        let header = self.header();
        header.data_type.get()
    }
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        'outer_loop: loop {
            ...
            for c in char_iter {
                ...
                if c == '\0' {
                    //empty strings don't break the loop
                    if !string.is_empty() {
                        break 'outer_loop;
                    }
                } else {
