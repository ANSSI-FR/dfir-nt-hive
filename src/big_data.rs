...
impl BigDataListItemRanges {
        ...
        B: SplitByteSlice,
    {
        ...
        let header: Ref<&[u8], BigDataHeader> = Ref::from_bytes(&hive.data[header_range]).unwrap();

        //removes automatic signature validation as we try to parse the field in a best effort mode
        ...
        if data_size > max_data_size {
            return Err(NtHiveError::InvalidSizeField {
                origin: "`segment_count` of the `BigDataHeader`".into(),
        ...
        let items_range = byte_subrange(&segment_list_cell_range, byte_count).ok_or_else(|| {
            NtHiveError::InvalidSizeField {
                origin: "range of Big Data list items".into(),
    ...
    pub fn validate_signature<B>(hive: &Hive<B>, header_cell_range: Range<usize>) -> Result<bool>
    where
        B: SplitByteSlice,
    {
        // The passed `header_cell_range` contains just the `BigDataHeader`.
    ...
    B: SplitByteSlice,
{
    ...
    pub(crate) fn has_valid_signature(
        hive: &'h Hive<B>,
        header_cell_range: Range<usize>,
    ) -> Result<bool> {
        BigDataListItemRanges::validate_signature(hive, header_cell_range)
    }
}
