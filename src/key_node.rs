...
impl KeyNodeItemRange {
        ...
        B: SplitByteSlice,
    {
        ...
        .ok_or_else(|| NtHiveError::InvalidSizeField {
            origin: "key node class name".into(),
    ...
    fn timestamp<'h, B>(&self, hive: &'h Hive<B>) -> u64
    where
        B: SplitByteSlice,
    {
        let header = self.header(hive);
    ...
    fn security_descriptor<'h, B>(&self, hive: &'h Hive<B>) -> Option<Result<Vec<u8>>>
    where
        B: SplitByteSlice,
    {
        let header = self.header(hive);
        let key_sec_offset = header.key_security_offset.get();

        let cell_range: Range<usize> = iter_try!(hive.cell_range_from_data_offset(key_sec_offset));

        static SK_HEADER_SIZE: usize = 16;
        // security key header:
        //    struct RegfCellSk {
        //        signature: [u8; 2],
        //        unk: [u8; 2], //
        ...
        //        refcount: [u8; 4],
        //    }
        //remove header size from the range
        let range: Range<usize> = Range {
            start: cell_range.start + SK_HEADER_SIZE,
            end: cell_range.end,
        };
        if range.is_empty() {
            Some(Ok(vec![]))
        } else {
            let bytes = &hive.data[range];
...
pub struct KeyNode<'h, B: SplitByteSlice> {
    pub hive: &'h Hive<B>,
    pub item_range: KeyNodeItemRange,
}
    ...
    B: SplitByteSlice,
{
    ...
    ///returns the security bytes
    pub fn security_descriptor(&self) -> Result<Vec<u8>> {
        match self.item_range.security_descriptor(self.hive) {
            Some(a) => a,
            None => Ok(vec![]),
        }
    }
...
mod tests {
    ...
    fn test_security_descriptor() {
        ...
        let root_key_node = hive.root_key_node().unwrap();
        let key_node = root_key_node.subkey("subpath-test").unwrap().unwrap();
        let decrp_1 = key_node.security_descriptor().unwrap();

        let sub_node = key_node
            .subpath("with-two-levels-of-subkeys\\subkey1\\\\subkey2")
            .unwrap()
            .unwrap();

        let decrp_2 = sub_node.security_descriptor().unwrap();
        assert_eq!(decrp_1, decrp_2);

        let sign = String::from_utf8_lossy(&decrp_2[0..2]);
        println!("{sign}")
    }

    #[test]
    fn test_security_descriptor_tiny() {
        let mut testhive = Vec::new();
        File::open("testdata/tiny.hve")
            .unwrap()
            .read_to_end(&mut testhive)
            .unwrap();

        let hive = Hive::new(testhive.as_ref()).unwrap();
        let root_key_node = hive.root_key_node().unwrap();
        let desc = root_key_node.security_descriptor().unwrap();
        assert_eq!(desc.len(), 268);
    }
